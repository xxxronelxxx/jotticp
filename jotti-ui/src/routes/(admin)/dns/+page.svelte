...
<svelte:head>
  <title>DNS Management — JottiCP</title>
</svelte:head>
...
<div class="flex flex-col h-full min-h-0" style="height: calc(100vh - 4rem);">
  ...
  <div class="flex items-center justify-between px-4 lg:px-6 py-3 border-b border-border bg-background shrink-0">
    <div>
      <h1 class="text-lg font-semibold text-foreground">DNS Management</h1>
      {#if !loading}
        <p class="text-xs text-muted-foreground">
          {zones.length} {zones.length === 1 ? 'zone' : 'zones'} · {totalRecords} total records
          {#if zonesWithIssues.length > 0}
            · <span class="text-amber-400">{zonesWithIssues.length} with issues</span>
          {:else}
      ...
      <div class="w-full md:w-[280px] shrink-0 flex flex-col border-b md:border-b-0 md:border-r border-border bg-card overflow-hidden">
        ...
        <div class="p-3 border-b border-border">
          <div class="relative">
            ...
            <input type="search" bind:value={zoneSearch} placeholder="Search zones…"
