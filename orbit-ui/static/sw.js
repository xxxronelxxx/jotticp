...
self.addEventListener('push', (event) => {
    ...
    try {
        ...
        data = { title: 'OrbitCP', body: event.data.text() };
    }
    ...
    const options = {
        ...
        icon:    '/icon-192.png',
        badge:   '/icon-192.png',
        tag:     data.tag    || 'orbitcp',
        data:    { url: data.url || '/dashboard' },
        actions: data.actions || [],
    };

    event.waitUntil(
        self.registration.showNotification(data.title || 'OrbitCP Alert', options)
    );
});
