<script lang="ts">
  ...
  const catalog: Plugin[] = [
    ...
    },
    {
      ...
      description: 'OWASP Core Rule Set 4.x + additional OrbitCP curated rules. Requires ModSecurity to be enabled on the web server.',
      version: '4.3.1',
      author: 'OrbitCP Team',
      homepage: null,
      icon_url: null,
...
<div class="space-y-6 page-content">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div>
      ...
      <p class="text-sm text-muted-foreground mt-0.5">Extend OrbitCP with community and official plugins</p>
    </div>
    {#if installedCount > 0}
      ...
      <div class="lg:col-span-3 bg-card border border-border rounded-xl p-6 space-y-5">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-muted-foreground">Upload a community or custom OrbitCP plugin archive</p>
          </div>
        </div>
          ...
          <div class="flex items-start gap-2.5">
            ...
            <div>
              ...
              <p class="text-xs text-amber-300/80 leading-relaxed">
                Only install plugins from trusted sources. Plugins run in a sandboxed environment but still have access to your server. OrbitCP signed plugins are verified before install.
              </p>
            </div>
          </div>
        </div>
