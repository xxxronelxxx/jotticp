<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      if (e.error === 'totp_already_enabled') {
        ...
        qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=otpauth://totp/OrbitCP:admin%40example.com?secret=JBSWY3DPEHPK3PXP%26issuer%3DOrbitCP`;
  ...
  function downloadCodes() {
    const text = `OrbitCP 2FA Backup Codes\nGenerated: ${new Date().toLocaleString()}\n\n${backupCodes.join('\n')}\n\nEach code can only be used once.`;
    const blob = new Blob([text], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'orbitcp-backup-codes.txt';
    a.click();
...
<svelte:head>
  <title>Two-Factor Authentication — OrbitCP</title>
</svelte:head>
