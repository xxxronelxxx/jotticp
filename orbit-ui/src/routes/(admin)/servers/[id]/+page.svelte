...
<svelte:head>
  <title>{server?.label ?? 'Server'} — OrbitCP</title>
</svelte:head>
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-muted-foreground mb-3">
              SSH into <span class="font-mono text-foreground">{server.ip}</span> and run the orbit-agent enrollment command to connect this server to OrbitCP.
            </p>
            <div class="bg-black/30 rounded-lg px-3 py-2 font-mono text-xs text-green-300 break-all">
              curl -s https://orbitcp.waytohosts.com/api/v1/servers/{server.id}/enroll | bash
            </div>
