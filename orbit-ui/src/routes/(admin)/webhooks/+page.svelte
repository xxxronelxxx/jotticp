...
<div class="p-4 lg:p-6 space-y-6">
  ...
  <div class="bg-card border border-border rounded-xl p-5">
    ...
    <div class="space-y-3 text-sm text-muted-foreground">
      <p>
        Every request OrbitCP sends to your endpoint is signed using
        <span class="text-foreground font-medium">HMAC-SHA256</span>. The signature is
        included in the <code class="font-mono text-xs bg-muted px-1 py-0.5 rounded text-foreground">X-OrbitCP-Signature</code> header
      ...
      <div class="bg-muted/30 border border-border rounded-lg p-3 font-mono text-xs text-foreground space-y-1">
        ...
        <p>assert request.headers["X-OrbitCP-Signature"] == hex(computed)</p>
      </div>
      <ul class="list-disc list-inside space-y-1 text-xs">
        <li>If you leave the secret blank, OrbitCP auto-generates a cryptographically random 32-byte secret.</li>
        <li>Rotate secrets by deleting and re-adding the webhook endpoint.</li>
        <li>OrbitCP will retry failed deliveries up to 3 times with exponential back-off.</li>
        <li>Each request includes an <code class="font-mono bg-muted px-1 py-0.5 rounded text-foreground">X-OrbitCP-Event</code> header naming the event type.</li>
      </ul>
    </div>
  </div>
</div>
    ...
    <div class="bg-card border border-border rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl">
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          />
          <p class="text-xs text-muted-foreground mt-1">Used to verify the <code class="font-mono bg-muted px-1 rounded">X-OrbitCP-Signature</code> header.</p>
        </div>
