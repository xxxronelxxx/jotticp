...
async fn compute_fingerprint(public_key: &str) -> ApiResult<String> {
    ...
    let tmp = format!("/tmp/orbitcp-key-{}.pub", Uuid::new_v4());
...
/// Sets `PasswordAuthentication no` in /etc/ssh/sshd_config.d/orbitcp.conf and reloads sshd.
...
async fn disable_password_auth(
    ...
    let sshd_conf = "/etc/ssh/sshd_config.d/orbitcp.conf";
    let conf_content = "# Managed by OrbitCP — do not edit manually\nPasswordAuthentication no\nChallengeResponseAuthentication no\n";
