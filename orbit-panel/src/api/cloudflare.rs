...
/// Sync all DNS records from OrbitCP → Cloudflare for this zone.
async fn sync_zone(
    State(state): State<Arc<AppState>>,
    claims: Claims,
    Path(id): Path<Uuid>,
    ...
    // Fetch all DNS records for this domain from OrbitCP DB
...
fn encrypt_token(token: &str, config: &crate::config::Config) -> String {
    ...
    let fallback = "orbitcp_default_key_change_me".to_string();
