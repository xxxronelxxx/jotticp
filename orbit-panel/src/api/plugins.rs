...
const PLUGIN_INSTALL_DIR: &str = "/var/orbitcp/plugins";
...
/// Upload a .tar.gz plugin archive containing `orbit-plugin.toml`.
async fn upload_plugin(
    ...
    // Extract and parse orbit-plugin.toml from the archive
...
async fn install_plugin(
    ...
    // Marketplace plugins are downloaded from the OrbitCP registry in prod.
    ...
    sqlx::query!(
        r#"INSERT INTO plugins (id, slug, name, version, author, description, enabled)
           VALUES ($1, $2, $3, '1.0.0', 'OrbitCP Team', 'Marketplace plugin', true)
...
/// Extract `orbit-plugin.toml` from a gzipped tar archive and parse it.
fn extract_manifest(data: &[u8]) -> anyhow::Result<PluginManifest> {
    ...
    for entry in archive.entries()? {
        ...
        let filename = path.file_name()
            .and_then(|n| n.to_str())
            .unwrap_or("");
        if filename == "orbit-plugin.toml" {
            let mut contents = String::new();
            entry.read_to_string(&mut contents)?;
            let manifest: PluginManifest = toml::from_str(&contents)
                .map_err(|e| anyhow::anyhow!("orbit-plugin.toml parse error: {}", e))?;
            return Ok(manifest);
        }
    }
    Err(anyhow::anyhow!("orbit-plugin.toml not found in archive root"))
}
