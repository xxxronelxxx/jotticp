...
const HARDENING_CONF: &str = "/etc/sysctl.d/99-orbitcp-hardening.conf";
...
pub struct HardeningStatus {
    /// True if /etc/sysctl.d/99-orbitcp-hardening.conf exists.
    pub applied:     bool,
...
async fn get_settings(
    ...
    Ok(Json(PanelSettings {
        panel_domain:       state.config.panel_domain.clone(),
...
async fn restart_service(
    ...
    // Spawn restart in background so the response is sent before orbit-panel itself may restart.
    let svc = name.clone();
