...
/// `jti` is a per-token unique ID. Blocklist key is `orbit:blocklist:{jti}` —
...
/// Generated at first run, stored in /etc/orbitcp/jwt_ec_key.pem (private, 0600).
...
async fn logout(
    ...
    if ttl > 0 {
        ...
        let mut conn = state.valkey.clone();
        let _: () = conn
            .set_ex(format!("orbit:blocklist:{}", claims.jti), "1", ttl)
            .await
            .unwrap_or(());
    }
...
async fn setup_totp(
    ...
    // Store pending secret in Valkey for 10 minutes
    {
        use redis::AsyncCommands;
        let mut conn = state.valkey.clone();
        let key = format!("orbit:totp_setup:{}", user_id);
        let _: () = conn.set_ex(&key, &secret_b32, 600u64).await.unwrap_or(());
    }

    let provisioning_uri = format!(
        "otpauth://totp/OrbitCP:{email}?secret={secret}&issuer=OrbitCP",
...
async fn confirm_totp(
    ...
    // Delete pending secret from Valkey
    {
        use redis::AsyncCommands;
        let mut conn = state.valkey.clone();
        let _: () = conn.del(format!("orbit:totp_setup:{}", user_id)).await.unwrap_or(());
    }
