...
async fn update_branding(
    ...
    // brand_name is NOT NULL — use panel_name if provided, else "OrbitCP"
    let brand_name = req.panel_name.as_deref().unwrap_or("OrbitCP").to_string();
...
async fn upload_logo(
    ...
    let logo_dir = format!("/etc/orbitcp/branding/{}", lookup_id);
...
fn logo_path_to_url(path: &str) -> String {
    ...
    // /etc/orbitcp/branding/{id}/logo.png -> /api/v1/static/branding/{id}/logo.png
    path.replace("/etc/orbitcp/branding/", "/api/v1/static/branding/")
}
