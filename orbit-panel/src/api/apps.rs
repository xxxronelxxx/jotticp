...
pub async fn dispatch_app_install_by_method(
    ...
    if runtime.git_init {
        ...
        let git_cmds = [
            ...
            format!(
                r#"sudo -u {unix_user} git -C {git_dir} commit -m "Initial OrbitCP install""#
            ),
        ];
...
/// OrbitCP release signing key pair.
...
/// `ORBIT_APPS_DIR` environment variable (default: `/etc/orbitcp/apps`).
///
...
pub fn load_app_manifests() -> Vec<AppManifest> {
    let apps_dir = std::env::var("ORBIT_APPS_DIR")
        .unwrap_or_else(|_| "/etc/orbitcp/apps".to_string());
...
// Known app manifests (loaded from /src/orbit-panel/apps/*.toml in production;
...
/// Manifests are loaded from `ORBIT_APPS_DIR` (default `/etc/orbitcp/apps`)
