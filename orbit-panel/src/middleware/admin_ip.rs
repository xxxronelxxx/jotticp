...
pub async fn admin_ip_check(
    ...
    // Behind nginx the TCP peer is always 127.0.0.1, so trust X-Forwarded-For /
    // X-Real-IP (set by our own nginx) ONLY when the immediate peer is loopback.
    let client_ip = {
        let peer_ip = peer.ip();
        if peer_ip.is_loopback() {
            req.headers()
                ...
                .map(|s| s.trim())
                .or_else(|| req.headers().get("x-real-ip").and_then(|v| v.to_str().ok()).map(|s| s.trim()))
