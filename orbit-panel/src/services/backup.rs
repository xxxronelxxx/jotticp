...
///
/// Backup layout:
///   /var/backups/orbitcp/{site_id}/{YYYY-MM-DD-HHMMSS}/
...
pub async fn backup_site(state: &Arc<AppState>, site_id: Uuid) -> anyhow::Result<()> {
    ...
    let backup_dir = PathBuf::from(format!("/var/backups/orbitcp/{}/{}", site_id, ts));
        ...
        if !to_keep.contains(&b.id) {
            ...
            if let Some(ref manifest) = b.manifest_path {
                ...
                if let Some(parent) = PathBuf::from(manifest).parent() {
                    if parent.starts_with("/var/backups/orbitcp") {
...
pub async fn restore_site(
    ...
    anyhow::ensure!(
        backup_dir.starts_with("/var/backups/orbitcp"),
