...
/// gRPC client types generated from proto/orbit.proto at build time (see build.rs).
pub mod agentproto { tonic::include_proto!("orbit"); }
...
// ── Remote provisioning (via orbit-agent gRPC) ────────────────────────────────

/// Build an mTLS gRPC channel to an orbit-agent (certs from /etc/orbitcp/agent/).
pub async fn agent_channel(agent_addr: &str) -> anyhow::Result<tonic::transport::Channel> {
    ...
    let ca   = tokio::fs::read("/etc/orbitcp/agent/ca.pem").await
        .map_err(|e| anyhow::anyhow!("read agent CA: {}", e))?;
    let cert = tokio::fs::read("/etc/orbitcp/agent/panel-client.pem").await
        .map_err(|e| anyhow::anyhow!("read panel client cert: {}", e))?;
    let key  = tokio::fs::read("/etc/orbitcp/agent/panel-client-key.pem").await
    ...
    tonic::transport::Channel::from_shared(format!("https://{}", agent_addr))
        ...
        .connect().await.map_err(|e| anyhow::anyhow!("Cannot connect to orbit-agent at {} over mTLS: {}", agent_addr, e))
}
...
async fn provision_via_agent(
    ...
    tracing::info!(
        ...
        server_id = %server_id,
        agent     = %agent_addr,
        "Provisioning via orbit-agent gRPC"
    );

    // Connect to orbit-agent over MUTUAL TLS (gRPC) via the shared helper.
    ...
    .await
    .map_err(|e| anyhow::anyhow!("orbit-agent CreateSite gRPC failed: {}", e))?
    .into_inner();

    if resp.status == "error" {
        anyhow::bail!("orbit-agent CreateSite error: {}", resp.error_msg);
    }
    tracing::info!(site_id = %job.site_id, server = %server.label, agent_user = %resp.unix_user,
        "orbit-agent CreateSite OK (gRPC mTLS)");
    Ok(())
}
...
fn write_cgroup_slice(domain: &str, unix_user: &str) -> anyhow::Result<()> {
    ...
    // Use conservative limits for community tier; orbit-agent overrides for Pro
    let content = format!(
        "[Unit]\n\
         Description=OrbitCP site cgroup slice for {domain}\n\
...
fn write_php_fpm_pool(job: &ProvisioningJob) -> anyhow::Result<()> {
    ...
    let config = format!(
        "; OrbitCP auto-generated PHP-FPM pool for {domain}\n\
         ; DO NOT EDIT — managed by orbit-panel. Changes will be overwritten.\n\
         [{pool}]\n\
         user  = {user}\n\
         group = {user}\n\
         \n\
         ...
         php_admin_value[opcache.file_cache]           = /var/orbitcp/opcache/{user}\n\
    ...
    for dir in &[
        ...
        format!("/var/orbitcp/opcache/{}", job.unix_user),
...
fn write_nginx_vhost(job: &ProvisioningJob) -> anyhow::Result<()> {
    ...
    let config = format!(
        "# OrbitCP auto-generated vhost for {domain}\n\
         # DO NOT EDIT — managed by orbit-panel.\n\
         server {{\n\
             listen 80;\n\
...
fn write_nginx_proxy_vhost(domain: &str, backend_port: u16) -> anyhow::Result<()> {
    ...
    let config = format!(
        "# OrbitCP auto-generated nginx proxy vhost for {domain} → :{port}\n\
         # DO NOT EDIT — managed by orbit-panel.\n\
         server {{\n\
             listen 80;\n\
             server_name {domain} www.{domain};\n\
...
fn write_apache_vhost(job: &ProvisioningJob) -> anyhow::Result<()> {
    ...
    let config = format!(
        "# OrbitCP auto-generated Apache vhost for {domain}\n\
         <VirtualHost *:8081>\n\
             ServerName  {domain}\n\
...
/// Dispatch a language runtime setup to the appropriate orbit-agent via gRPC
/// (or locally if no server_id is given).
///
/// `runtime` must be one of "python" | "nodejs" | "ruby" | "dotnet".
/// The actual work is done inside orbit-agent/src/handlers/runtime.rs.
...
pub fn deprovision_runtime_sync(domain: &str, unix_user: &str, prev_runtime: &str) -> anyhow::Result<()> {
    ...
    match prev_runtime {
        "nodejs" => {
            let unit = format!("orbitcp-nodejs-{}", domain);
        ...
        "python" => {
            let unit = format!("orbitcp-python-{}", domain);
            let main_pid: Option<u32> = std::process::Command::new("systemctl")
                .args(["show", &unit, "--property=MainPID", "--value"])
                .output()
                .ok()
...
pub fn provision_nodejs_sync(domain: &str, unix_user: &str, port: i32) -> anyhow::Result<()> {
    ...
    let log_dir   = format!("/var/log/orbitcp/sites/{}", domain);
    let unit_name = format!("orbitcp-nodejs-{}", domain);
        ...
        && !std::path::Path::new(&format!("{}/package.json", docroot)).exists()
    {
        let placeholder = format!(
            "// OrbitCP — placeholder server for {domain}\n\
             // Replace this file with your real Node.js application.\n\
             const http = require('http');\n\
             const PORT = process.env.PORT || {port};\n\
             http.createServer((req, res) => {{\n\
               res.writeHead(200, {{'Content-Type': 'text/plain'}});\n\
               res.end('OrbitCP: Deploy your app to /home/{unix_user}/public_html/\\n');\n\
    ...
    let unit_content = format!(
        "[Unit]\n\
         Description=OrbitCP Node.js app: {domain}\n\
...
pub fn provision_python_sync(
    ...
    let docroot     = format!("/home/{}/public_html",  unix_user);
    let log_dir     = format!("/var/log/orbitcp/sites/{}", domain);
    let unit_name   = format!("orbitcp-python-{}", domain);
    let unit_path   = format!("/etc/systemd/system/{}.service", unit_name);
    ...
    let unit_content = format!(
        "[Unit]\n\
         Description=OrbitCP Python app: {domain}\n\
         After=network.target\n\
         \n\
         [Service]\n\
         Type=simple\n\
...
fn write_runtime_proxy_vhost(domain: &str, port: u16) -> anyhow::Result<()> {
    ...
    let config = format!(
        "# OrbitCP runtime proxy vhost: {domain} → 127.0.0.1:{port}\n\
         # DO NOT EDIT — managed by orbit-panel.\n\
         server {{\n\
             listen 80;\n\
             server_name {domain} www.{domain};\n\
...
// run_cmd_as and get_uid_gid removed: orbit-panel lacks CAP_SETUID (NoNewPrivileges=true
...
pub async fn setup_runtime_site(
    ...
    // If a remote server is specified, call orbit-agent via HTTP (gRPC stub).
    ...
    if let Some(sid) = server_id {
        ...
        let resp = state.http
            ...
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("orbit-agent setup_runtime call failed: {}", e))?;

        if !resp.status().is_success() {
            let status = resp.status();
            let text   = resp.text().await.unwrap_or_default();
            anyhow::bail!("orbit-agent setup_runtime returned {}: {}", status, text);
        }

        tracing::info!(site_id = %site_id, %runtime, "Remote runtime provisioning OK");
    } else {
        // Local provisioning — orbit-agent runs on the same machine.
        // The actual setup logic lives in orbit-agent; here we call the
        ...
        let resp = state.http
            ...
            .send()
            .await
            .map_err(|e| anyhow::anyhow!("local orbit-agent setup_runtime call failed: {}", e))?;

        if !resp.status().is_success() {
            let status = resp.status();
            let text   = resp.text().await.unwrap_or_default();
            anyhow::bail!("local orbit-agent setup_runtime returned {}: {}", status, text);
        }
