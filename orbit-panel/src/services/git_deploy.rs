...
const GIT_ROOT: &str = "/var/orbitcp/git";

// ── Public API ────────────────────────────────────────────────────────────────

/// Enable git push-to-deploy for a site.
///
/// Creates `/var/orbitcp/git/SITEID.git` (bare repo) and installs a post-receive
...
pub async fn enable_git_deploy(state: &Arc<AppState>, site_id: Uuid) -> Result<String> {
    ...
    // Ensure the orbitcp user owns the bare repo
    run_cmd("chown", &["-R", "orbitcp:orbitcp", &repo_path])
...
fn build_hook(site_id: &str, unix_user: &str, domain: &str) -> String {
    format!(
        r#"#!/bin/bash
# OrbitCP post-receive deploy hook for site {site_id} ({domain})
# Generated by orbit-panel — do not edit manually.
...
while read oldrev newrev refname; do
    if [ "$refname" = "refs/heads/main" ] || [ "$refname" = "refs/heads/master" ]; then
        echo "OrbitCP: Deploying ${{newrev:0:8}} to $SITE_HOME..."
        ...
        # Record the deployment in orbit-panel
        ...
        echo "OrbitCP: Deploy of ${{newrev:0:8}} complete!"
    fi
done
"#,
        site_id   = site_id,
