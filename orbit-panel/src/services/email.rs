...
// These functions are called by orbit-panel API handlers and background jobs.
// The actual email server management (Postfix/Dovecot config writes) is done
// by orbit-mail daemon at 127.0.0.1:5354. This module provides the HTTP
// client wrapper.
...
/// Create a mail domain in orbit-mail (Postfix virtual_domains + DKIM key setup).
pub async fn create_mail_domain(domain: &str) -> anyhow::Result<()> {
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| anyhow::anyhow!("orbit-mail unreachable: {}", e))?;

    if !resp.status().is_success() {
        let body = resp.text().await.unwrap_or_default();
        return Err(anyhow::anyhow!("orbit-mail error creating domain {}: {}", domain, body));
    }

    tracing::info!(%domain, "Mail domain created via orbit-mail");
    Ok(())
}
...
pub async fn delete_mail_domain(domain: &str) -> anyhow::Result<()> {
    ...
    if !resp.status().is_success() && resp.status() != reqwest::StatusCode::NOT_FOUND {
        let body = resp.text().await.unwrap_or_default();
        return Err(anyhow::anyhow!("orbit-mail error deleting domain {}: {}", domain, body));
    }

    tracing::info!(%domain, "Mail domain deleted via orbit-mail");
    Ok(())
}
...
pub async fn create_mailbox(domain: &str, username: &str, password: &str, quota_mb: u32) -> anyhow::Result<()> {
    ...
    if !resp.status().is_success() {
        let body = resp.text().await.unwrap_or_default();
        return Err(anyhow::anyhow!("orbit-mail error creating mailbox {}@{}: {}", username, domain, body));
    }

    tracing::info!(%username, %domain, "Mailbox created via orbit-mail");
    Ok(())
}
...
pub async fn delete_mailbox(domain: &str, username: &str) -> anyhow::Result<()> {
    ...
    if !resp.status().is_success() && resp.status() != reqwest::StatusCode::NOT_FOUND {
        let body = resp.text().await.unwrap_or_default();
        return Err(anyhow::anyhow!("orbit-mail error deleting mailbox {}@{}: {}", username, domain, body));
    }

    tracing::info!(%username, %domain, "Mailbox deleted via orbit-mail");
    Ok(())
}
...
pub async fn enable_dkim(domain: &str) -> anyhow::Result<String> {
    ...
    if !resp.status().is_success() {
        let body = resp.text().await.unwrap_or_default();
        return Err(anyhow::anyhow!("orbit-mail error enabling DKIM for {}: {}", domain, body));
    }

    let body: serde_json::Value = resp.json().await
        .map_err(|e| anyhow::anyhow!("Invalid orbit-mail response: {}", e))?;
    ...
    tracing::info!(%domain, "DKIM enabled via orbit-mail");
    Ok(dns_value)
}
...
/// Check if orbit-mail is healthy.
