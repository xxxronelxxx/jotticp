...
async fn process_next_job(state: &Arc<AppState>) -> anyhow::Result<bool> {
    ...
    match job_type.as_str() {
        ...
        // start_valkey is enqueued by dispatch_provision_valkey above; orbit-agent
...
async fn dispatch_delete_backup_files(_state: &Arc<AppState>, job: &serde_json::Value) {
    ...
    match p.parent() {
        Some(dir) if dir.starts_with("/var/backups/orbitcp") => {
...
async fn dispatch_reload_php_pool(state: &Arc<AppState>, job: &serde_json::Value) {
    ...
    match std::fs::read_to_string(&pool_path) {
        Ok(conf) => {
            const BEGIN: &str = "; >>> orbitcp-custom (managed; do not edit)";
            const END:   &str = "; <<< orbitcp-custom";
...
async fn dispatch_sync_cron(state: &Arc<AppState>, job: &serde_json::Value) {
    ...
    let mut tab = String::from("# Managed by OrbitCP\n");
...
async fn dispatch_revoke_ssl(state: &Arc<AppState>, job: &serde_json::Value) {
    ...
    let dir = format!("/etc/orbitcp/ssl/{}", domain);
...
/// Dispatch a language runtime provisioning job to orbit-agent via gRPC.
///
...
/// orbit-agent and updates the site status on completion.
...
///
/// Writes /etc/orbitcp/valkey/SITEID.conf with:
...
///
/// Then enqueues a start_valkey job for orbit-agent.
async fn dispatch_provision_valkey(state: &Arc<AppState>, job: &serde_json::Value) {
    ...
    // Signal orbit-agent via job queue to start the Valkey service
    {
        use redis::AsyncCommands;
...
///
/// Pro-only.  Writes /etc/orbitcp/surrealdb/SITEID.env and a systemd unit
...
async fn dispatch_provision_surrealdb(state: &Arc<AppState>, job: &serde_json::Value) {
    ...
    let data_path    = format!("/var/orbitcp/surrealdb/{}", site_id);
    let env_dir      = "/etc/orbitcp/surrealdb";
    let env_path     = format!("{}/{}.env", env_dir, site_id);

    let env_content = format!(
        "# OrbitCP SurrealDB env — site {site_id}\n\
    ...
    let unit_content = format!(
        "[Unit]\n\
         Description=OrbitCP SurrealDB — site {site_id}\n\
...
/// Forward the start_valkey job to orbit-agent (or start locally if no server_id).
///
/// orbit-agent is responsible for executing `valkey-server /etc/orbitcp/valkey/SITEID.conf`
...
async fn dispatch_start_valkey(state: &Arc<AppState>, job: &serde_json::Value) {
    ...
    if !config_path.starts_with("/etc/orbitcp/valkey/") {
        tracing::error!(
    ...
    tracing::info!(site_id = %site_id, config_path = %config_path, "Dispatching start_valkey to orbit-agent");
...
fn load_app_toml_raw(app_id: &str) -> Option<toml::Value> {
    let apps_dir = std::env::var("ORBIT_APPS_DIR")
        .unwrap_or_else(|_| "/opt/orbitcp/apps".to_string());
    ...
    for (i, step) in steps.iter().enumerate() {
        ...
        match step_type {
            "cli" => {
                ...
                // Drop privileges via CommandExt::uid/gid — orbit-panel runs as root
