...
pub struct Config {
    ...
    /// Base URL for the webmail UI (e.g. https://orbitcp.waytohosts.com/webmail).
    /// Used when generating SSO login links. Set via WEBMAIL_URL env var.
    pub webmail_url:             String,
    pub smtp_from:               String,
    /// Base URL for the internal orbit-mail daemon (default: http://127.0.0.1:5354)
    pub orbit_mail_url:          String,
    /// Base URL for the internal orbit-dns daemon (default: http://127.0.0.1:5353)
    ...
    /// Base URL for the orbit-agent gRPC/HTTP control endpoint on managed servers.
...
impl Config {
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        let jwt_ec_private_key_path = std::env::var("JWT_EC_PRIVATE_KEY_PATH")
            .unwrap_or_else(|_| "/etc/orbitcp/jwt_ec_key.pem".into());

        let jwt_ec_public_key_path = std::env::var("JWT_EC_PUBLIC_KEY_PATH")
            .unwrap_or_else(|_| "/etc/orbitcp/jwt_ec_key.pub.pem".into());
