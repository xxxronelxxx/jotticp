...
$data = $redis->get('orbit:pma:' . $token);
...
$redis->del('orbit:pma:' . $token); // single-use
...
function render(string $title, string $msg, string $type): string {
    ...
    return '<!DOCTYPE html><html><head><meta charset=UTF-8><title>' . htmlspecialchars($title) . ' — OrbitCP</title>'
