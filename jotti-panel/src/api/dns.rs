...
// urlencoding is used by email-auth handler to percent-encode domain names in jotti-mail URL
...
async fn setup_email_auth(
    ...
    // Queue DKIM key generation via jotti-agent
    {
        use redis::AsyncCommands;
        let job = serde_json::json!({
...
//   2. Calls jotti-mail to generate a DKIM keypair
...
async fn setup_email_auth_full(
    ...
    // --- 2. Call jotti-mail to generate DKIM keypair ---
    let mail_url = format!(
    ...
    let (dkim_dns_value, dkim_status) = match state.http.post(&mail_url).send().await {
        Ok(resp) if resp.status().is_success() => {
            match resp.json::<serde_json::Value>().await {
                ...
                Err(e) => {
                    tracing::warn!(error = %e, "jotti-mail DKIM response parse failed");
                    (None, "parse_error".to_string())
                }
            }
        }
        Ok(resp) => {
            tracing::warn!(status = %resp.status(), "jotti-mail DKIM generation failed");
            (None, format!("jotti_mail_error_{}", resp.status().as_u16()))
        }
        Err(e) => {
            tracing::warn!(error = %e, "jotti-mail DKIM call failed (will queue async)");
