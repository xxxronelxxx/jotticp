...
///
/// jotti-mail daemon implements the /internal/email/ip-bind endpoints
...
/// Call jotti-mail to bind `domain` to `ip_address` for outbound SMTP.
...
/// Call jotti-mail to remove the IP binding for a domain.
...
/// - For each domain: calls jotti-mail to set smtp_bind_address.
...
async fn assign_ip(
    ...
    for domain in &body.domains {
        ...
        // Call jotti-mail to bind the domain to this IP
...
/// Calls jotti-mail to remove the smtp_bind_address config.
#[instrument(skip(state))]
async fn remove_ip_assignment(
    ...
    // Remove binding from jotti-mail
