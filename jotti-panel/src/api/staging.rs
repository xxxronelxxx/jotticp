...
/// Run a shell command via jotti-agent HTTP endpoint.
...
async fn create_staging(
    ...
    // Although site.unix_user was set by JottiCP at provisioning time, we validate
    ...
    run_shell_cmd(
        ...
        site_id,
        &format!(
            "jotti-agent vhost create --domain {domain} --user {user} --webroot /home/{user}/public_html",
...
async fn delete_staging(
    ...
    run_shell_cmd(
        ...
        site_id,
        &format!(
            "jotti-agent vhost delete --domain {} 2>/dev/null || true",
