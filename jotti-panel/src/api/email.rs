...
async fn set_smarthost(
    ...
    // jotti-mail uses POST for smarthost creation/update
...
async fn get_queue(
    ...
    // Degrade gracefully when jotti-mail sidecar is not deployed: return
    ...
    match state.http.get(&url).send().await {
        Ok(resp) => {
            ...
            let body = resp.bytes().await
                .map_err(|e| ApiError::Internal(anyhow::anyhow!("jotti-mail body: {e}")))?;
            Ok((status, [(CONTENT_TYPE, "application/json")], body).into_response())
        }
        Err(_) => {
            let empty = r#"{"messages":[],"total":0,"jotti_mail_available":false}"#;
...
//
// These forward requests to jotti-mail at state.config.orbit_mail_url.
// They pass the raw body through and return the jotti-mail JSON response.
...
async fn proxy_get(state: &AppState, url: &str) -> ApiResult<axum::response::Response> {
    ...
    let resp = state.http.get(url).send().await
        .map_err(|e| ApiError::Internal(anyhow::anyhow!("jotti-mail GET: {e}")))?;
...
async fn proxy_put(state: &AppState, url: &str, body: axum::body::Bytes) -> ApiResult<axum::response::Response> {
    ...
    let resp = state.http.put(url)
        ...
        .send()
        .await
        .map_err(|e| ApiError::Internal(anyhow::anyhow!("jotti-mail PUT: {e}")))?;
...
async fn proxy_post_body(state: &AppState, url: &str, body: axum::body::Bytes) -> ApiResult<axum::response::Response> {
    ...
    let resp = state.http.post(url)
        ...
        .send()
        .await
        .map_err(|e| ApiError::Internal(anyhow::anyhow!("jotti-mail POST: {e}")))?;
...
async fn proxy_delete(state: &AppState, url: &str) -> ApiResult<axum::response::Response> {
    ...
    let resp = state.http.delete(url).send().await
        .map_err(|e| ApiError::Internal(anyhow::anyhow!("jotti-mail DELETE: {e}")))?;
