...
async fn test_webhook(
    ...
    let payload = json!({
        ...
        "data":     { "message": "JottiCP webhook test" },
    });

    let mut req_builder = state.http
        .post(&row.url)
        .header("Content-Type", "application/json")
        .header("User-Agent", "JottiCP-Webhook/1.0")
    ...
    if let Some(secret) = row.secret {
        ...
        if let Ok(mut mac) = HmacSha256::new_from_slice(secret.as_bytes()) {
            ...
            req_builder = req_builder.header("X-JottiCP-Signature", format!("sha256={}", sig));
        }
    }
        ...
        tokio::spawn(async move {
            ...
            if let Some(sec) = secret {
                ...
                if let Ok(mut mac) = HmacSha256::new_from_slice(sec.as_bytes()) {
                    mac.update(body.as_bytes());
                    let sig = hex::encode(mac.finalize().into_bytes());
                    rb = rb.header("X-JottiCP-Signature", format!("sha256={}", sig));
                }
            }
