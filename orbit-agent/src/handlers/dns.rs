...
async fn do_create_dns_zone(
    ...
    let ns1 = if ns1.is_empty() { "ns1.orbitcp.local." } else { ns1 };
    let ns2 = if ns2.is_empty() { "ns2.orbitcp.local." } else { ns2 };
    ...
    let pdns_api_key = std::env::var("PDNS_API_KEY")
        .unwrap_or_else(|_| "orbitcp-pdns-key".to_string());
...
fn pdns_api() -> (String, String) {
    ...
    let key = std::env::var("PDNS_API_KEY").unwrap_or_else(|_| "orbitcp-pdns-key".to_string());
    (url, key)
}
