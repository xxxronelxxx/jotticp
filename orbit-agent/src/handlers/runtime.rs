...
/// Write a systemd unit to /etc/systemd/system/orbit-site-SITEID.service,
...
fn write_and_enable_systemd_unit(site_id: &str, unit_content: &str) -> Result<(), Status> {
    ...
    let unit_name = format!("orbit-site-{}", safe_id);
...
fn write_nginx_upstream(site_id: &str, port: u16) -> Result<(), Status> {
    ...
    let content = format!(
        "# orbit-agent managed — do not edit manually\n\
...
fn do_setup_python(site_id: &str, unix_user: &str, python_bin: &str, port: u16) -> Result<(), Status> {
    ...
    let unit = format!(
        "[Unit]\n\
         Description=OrbitCP Python site — {unix_user}\n\
         ...
         Restart=on-failure\n\
         RestartSec=5s\n\
         # Cgroup isolation — site resource limits enforced by orbit-cron\n\
...
fn do_setup_nodejs(site_id: &str, unix_user: &str, node_version: &str, port: u16) -> Result<(), Status> {
    ...
    let unit = format!(
        "[Unit]\n\
         Description=OrbitCP Node.js site — {unix_user}\n\
         After=network.target\n\
         \n\
         [Service]\n\
         Type=simple\n\
...
fn do_setup_ruby(site_id: &str, unix_user: &str, ruby_version: &str, port: u16) -> Result<(), Status> {
    ...
    let default_puma_config = format!(
        "# Puma configuration — generated by OrbitCP\n\
         port        {port}\n\
    ...
    let unit = format!(
        "[Unit]\n\
         Description=OrbitCP Ruby/Puma site — {unix_user}\n\
         After=network.target\n\
         \n\
         [Service]\n\
         Type=simple\n\
...
fn do_setup_dotnet(site_id: &str, unix_user: &str, dotnet_version: &str, port: u16) -> Result<(), Status> {
    ...
    let env_file = format!("/etc/orbitcp/dotnet/{}.env", site_id);
    let env_dir  = "/etc/orbitcp/dotnet";
    ...
    let unit = format!(
        "[Unit]\n\
         Description=OrbitCP .NET {dotnet_version} site — {unix_user}\n\
         After=network.target\n\
         \n\
         [Service]\n\
         Type=simple\n\
