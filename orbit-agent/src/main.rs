...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        .with(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "orbit_agent=info,tonic=warn".into()),
        )
    ...
    tracing::info!(
        ...
        %addr,
        "orbit-agent starting"
    );

    // ── mTLS configuration ────────────────────────────────────────────────────
    let cert_path = std::env::var("AGENT_CERT_PATH")
        .unwrap_or_else(|_| "/etc/orbitcp/agent/agent.pem".into());
    let key_path = std::env::var("AGENT_KEY_PATH")
        .unwrap_or_else(|_| "/etc/orbitcp/agent/agent-key.pem".into());
    let ca_path = std::env::var("AGENT_CA_PATH")
        .unwrap_or_else(|_| "/etc/orbitcp/agent/ca.pem".into());
    ...
    let journal_path = std::env::var("ORBIT_JOURNAL_PATH")
        .unwrap_or_else(|_| "/var/lib/orbitcp/agent/journal.db".into());
    ...
    tracing::info!("orbit-agent listening with mTLS on {}", addr);
    ...
    tracing::info!("orbit-agent shutdown complete");
    Ok(())
}
