/// orbit-mail — Postfix + Dovecot + rspamd management daemon
///
...
pub fn postfix_rate_limit_maincf_fragment() -> &'static str {
    r#"# ── OrbitCP: policyd-rate-limit integration ──────────────────────────────────
...
async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "service": "orbit-mail" }))
}
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("orbit_mail=info".parse().unwrap()),
        )
        .init();
    ...
    info!(listen = %listen_addr, "orbit-mail starting");
    ...
    info!(addr = %listen_addr, "orbit-mail listening");
