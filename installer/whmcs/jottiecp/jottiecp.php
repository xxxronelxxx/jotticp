...
function orbitcp_MetaData(): array {
    return [
        'DisplayName'    => 'JottiCP Web Hosting',
        'APIVersion'     => '1.1',
        'RequiresServer' => true,
    ];
}
...
function orbitcp_ConfigOptions(): array {
    return [
        'package_name' => [
            'FriendlyName' => 'JottiCP Package Name',
            'Type'         => 'text',
            'Size'         => 25,
            'Default'      => 'basic',
            'Description'  => 'The JottiCP package/plan name to assign',
        ],
        'disk_gb' => [
...
/**
 * Make an authenticated request to the JottiCP REST API.
 *
...
function _orbitcp_api(array $params, string $method, string $endpoint, array $data = []): array {
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_HTTPHEADER     => [
            ...
            'X-JottiCP-Client: whmcs-module/0.0.1',
        ],
        CURLOPT_TIMEOUT        => 30,
    ...
    if ($httpCode >= 400) {
        ...
        return ['error' => "JottiCP API error ({$httpCode}): {$msg}"];
    }
 ...
 *
 * Creates a JottiCP user then a site under that user. Stores the JottiCP
...
function orbitcp_CreateAccount(array $params): string {
    ...
    // 3. Persist the JottiCP user ID in WHMCS so other hooks can reference it
    if ($userId) {
        try {
...
function orbitcp_SuspendAccount(array $params): string {
    ...
    if (empty($userId)) {
        return 'error: No JottiCP user ID on record — account may not have been provisioned via this module';
    }
...
function orbitcp_UnsuspendAccount(array $params): string {
    $userId = $params['username'];
    if (empty($userId)) {
        return 'error: No JottiCP user ID on record';
    }
 ...
 *
 * Deletion order: sites → user. This matches JottiCP's constraint that
 ...
 *
 * Updates the disk quota and plan name on the JottiCP user record.
 *
 * @return string "success" or "error: <message>"
 */
...
function orbitcp_UsageUpdate(array $params): array {
    ...
    return [
        ...
        'bwlimit'   => 0, // JottiCP does not enforce hard bandwidth caps
    ];
}
 ...
 * impersonation token (5-minute TTL) issued by JottiCP.
 *
