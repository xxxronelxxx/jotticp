/// orbit — OrbitCP command-line interface
///
/// Reads base URL from ~/.orbitcp/config.toml or ORBIT_URL env var.
...
const KEYRING_SERVICE: &str = "orbitcp";
...
fn config_path() -> PathBuf {
    ...
    home.join(".orbitcp").join("config.toml")
}
...
#[command(
    name = "orbit",
    about = "OrbitCP control panel CLI",
...
#[derive(Subcommand)]
enum Commands {
    /// Authenticate with the OrbitCP panel
    Login {
        #[arg(long)]
        url: Option<String>,
        #[arg(long)]
