/// jotti-dns — PowerDNS 4.9 REST API proxy daemon
///
...
async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "service": "jotti-dns" }))
}
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("jotti_dns=info".parse().unwrap()),
        )
        .init();
    ...
    info!(addr = %listen_addr, "jotti-dns listening");
