/// jotti-cron — background scheduler daemon
///
...
///   Task 4 (every 30s):   Collect metrics from jotti-agent via gRPC
///
...
// ─────────────────────────────────────────────────────────────────────────────
// gRPC generated code (jotti.proto)
// ─────────────────────────────────────────────────────────────────────────────
...
use jotti_proto::{agent_client::AgentClient, StreamMetricsRequest};
...
async fn send_dnsbl_alert_email(to: &str, domain: &str, ip: &str, dnsbl: &str) {
    let subject = format!("JottiCP DNSBL Alert: {} ({}) listed in {}", domain, ip, dnsbl);
    let body = format!(
        "JottiCP DNSBL Check Alert\n\
         ─────────────────────────\n\
         Domain : {domain}\n\
         IP     : {ip}\n\
         DNSBL  : {dnsbl}\n\
...
// ─────────────────────────────────────────────────────────────────────────────
// Task 4: Metrics collection from jotti-agent (every 30 seconds)
// ─────────────────────────────────────────────────────────────────────────────
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("jotti_cron=info".parse().unwrap()),
        )
        .init();

    info!("jotti-cron starting");
    ...
    tokio::select! {
        _ = shutdown_signal() => {
            info!("jotti-cron received shutdown signal, stopping");
        }
        result = t1 => {
